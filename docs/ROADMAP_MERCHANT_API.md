...
Рабочее локальное ТЗ на текущий docs-stage ведётся отдельно в:

- `docs/TZ_MERCHANT_API_PUBLIC_DOCS.md`
...
Выбранное решение на docs-stage от 2026-05-19:

- source of truth остаётся в репозитории как `docs/api/merchant/openapi.yaml`;
- в первой поставке spec отдаём как public static file URL из темы;
- `Redoc` и `Swagger UI` подключают spec по этому URL;
- если позже понадобится скрыть файловый layout темы, можно без смены контракта перевести spec на отдельный public route.

Причина выбора:

- это проще для текущего WordPress-theme стека;
- не требует дополнительного transport-layer ради одного YAML-файла;
- не мешает позже заменить способ отдачи spec без переписывания самих docs pages.
...
### Фактический статус на 2026-05-19
...
- Live self-test теперь отдельно подтверждён и для компании с `RUB` contour (`paymentAmount`):
  - `POST /merchant/invoices` создал live invoice на `5000.00 RUB`;
  - повтор того же запроса вернул `idempotent_replay = true`;
  - повтор с тем же `external_order_id`, но другим amount дал `409 conflict`;
  - order detail открылся только в своём merchant/company scope.
- Webhooks выведены из критического пути текущего MVP и отложены до отдельного решения.
- Этап 6 завершён server-side:
  - `OpenAPI` зафиксирован как source of truth;
  - raw spec опубликован как public static file URL;
  - публичные pages `merchant-api` и `merchant-api-console` созданы миграцией;
  - login guards обновлены так, чтобы docs pages были реально публичными.
...
- не потерять ни один из уже реализованных merchant API endpoint-ов при описании контракта.

Действия:

- зафиксировать отдельное локальное ТЗ для docs-stage;
- создать `docs/api/merchant/openapi.yaml`;
- описать в нём все endpoints, security scheme, DTO, enums, errors;
- описать в spec оба варианта `RUB_USDT`:
  - `orderAmount`, когда company contour работает через `USDT`;
  - `paymentAmount`, когда company contour работает через `RUB`;
- добавить helper public spec URL;
- создать page template для docs page;
- создать page template для console page;
...
- встроить `Redoc CE` без build pipeline;
- встроить `Swagger UI` без build pipeline;
- зафиксировать в spec примеры Bearer auth, list/read/create flow и ошибки `401/403/404/409/422`.

Проверка:

- docs page открывается без логина;
- console page открывается без логина;
- страницы создаются миграцией;
- spec соответствует реальным endpoint-ам;
- raw spec доступен по public URL;
- документация рендерится автоматически из OpenAPI.

Провал:

- docs page редиректит на `/authorization`;
- console page редиректит на `/authorization`;
- page создана вручную, а не миграцией;
- raw spec URL недоступен публично;
- OpenAPI расходится с фактическим API.

Подэтапы docs-stage на 2026-05-19:

1. Контракт
   - собрать `openapi.yaml` по уже работающим endpoint-ам;
   - зафиксировать envelopes, auth, DTO и примеры ошибок.
2. Public surface
   - поднять публичный URL для raw spec;
   - сделать helpers URL-ов docs/console/spec.
3. Pages
   - сделать публичную страницу `merchant-api` на `Redoc`;
   - сделать публичную страницу `merchant-api-console` на `Swagger UI`;
   - добавить навигацию между ними и ссылку на raw spec.
4. Platform wiring
   - создать pages миграцией;
   - добавить slug'и в `force-login` allowlist;
   - убедиться, что page templates работают без backoffice-auth.
5. QA
   - проверить `200 OK` на raw spec;
   - проверить отсутствие redirect на login;
   - проверить, что `Redoc`/`Swagger UI` читают один и тот же spec.

Фактический статус на 2026-05-19:

- создан `docs/api/merchant/openapi.yaml`;
- выбран и реализован public static spec URL из темы;
- созданы page templates:
  - `page-merchant-api.php`
  - `page-merchant-api-console.php`
- создана миграция `0080_create_merchant_api_public_docs_pages.php`;
- slug'и `merchant-api` и `merchant-api-console` добавлены в `includes/force-login.php`;
- найден и исправлен второй login-guard в `inc/security.php`, который изначально продолжал редиректить публичные docs pages на `/authorization`;
- self-test подтвердил:
  - `200 OK` на raw spec URL;
  - создание WordPress pages через миграцию;
  - `200 OK` на `GET /merchant-api/`;
  - `200 OK` на `GET /merchant-api-console/`;
  - наличие в HTML контейнеров `merchant-api-redoc` и `merchant-api-swagger`;
  - доступность CDN bundles для `Redoc` и `Swagger UI`.
...
- read/write rate limit реально срабатывает и возвращает `429` c `Retry-After`.
...
- включён per-client rate limit:
  - `read`: `120` запросов / `60` секунд;
  - `write`: `20` запросов / `60` секунд;
- auth-layer теперь пишет отдельные deny events для:
...
- live smoke подтвердил:
  - `401` на invalid token;
  - `401` на revoked token;
  - `403 merchant_pending` на pending merchant token;
  - `403 contour_disabled` на отключённое направление;
  - `404` на cross-scope order detail;
  - `429 rate_limit_exceeded` на 121-м read request;
  - отсутствие raw token и `Authorization` leakage в `debug.log` и `crm_audit_log`;
  - успешный `paymentAmount` create/replay/conflict flow на компании с `fintech_pay2day_order_currency = RUB`.
- отдельный live кейс `merchant_blocked` не был прогнан только потому, что на момент smoke не нашлось готового blocked-fixture мерчанта; сам deny-path в auth-layer покрыт кодом и логированием.
