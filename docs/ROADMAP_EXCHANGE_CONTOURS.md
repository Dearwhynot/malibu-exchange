...
- Для company fintech-create flows должен быть один и тот же amount contour в web и в operator bot:
  - если `fintech_active_provider = kanyon` и `fintech_pay2day_order_currency = RUB`, оператор вводит RUB, а ордер создаётся через `paymentAmount`;
  - если `fintech_pay2day_order_currency = USDT`, остаётся legacy-ввод USDT через `orderAmount`;
  - нельзя держать отдельную трактовку суммы для `create-order` и для operator bot.
- Для company fintech-create flows должен быть один и тот же `Ext Order ID` / payment purpose contour в web и в operator bot:
  - базовое значение брать из company setting `fintech_pay2day_default_payment_purpose`;
  - если в одном contour появится ручная замена значения перед созданием ордера, второй contour должен работать по той же модели;
  - нельзя оставлять web на company default payment purpose, а operator bot на отдельном техническом `merchant_order_id` или другой схеме.
