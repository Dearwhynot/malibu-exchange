<?php
/*
Template Name: Root Merchants Page
Slug: root-merchants
*/
...
$telegram_invite_status = $current_company_id > 0 ? crm_telegram_get_configuration_status( $current_company_id ) : [
	'invite_ready'   => false,
	'is_configured'  => false,
	'blocked_reason' => 'Инвайты доступны только в контексте компании.',
	'missing_fields' => [],
	'callback_url'   => '',
];
$companies           = crm_get_all_companies_list();
...
$nonce_delete   = wp_create_nonce( 'me_merchants_delete' );
		...
		<div class="content">
			...
			<div class="jumbotron" data-pages="parallax">
				<div class="container-fluid container-fixed-lg sm-p-l-0 sm-p-r-0">
					<div class="inner">
						<ol class="breadcrumb">
							<li class="breadcrumb-item"><a href="<?php echo esc_url( malibu_exchange_get_root_dashboard_url() ); ?>">Сводка</a></li>
			...
			<div class="container-fluid container-fixed-lg mt-4">
				...
				<div class="m-b-15">
					...
					<p class="hint-text m-b-0">
						Root-only агрегированный контур мерчантов по всем компаниям с hard delete только для dev-сценариев. Новые мерчанты должны появляться только через Telegram invite и запуск бота по персональной ссылке.
					</p>
				</div>
				...
				<div class="card card-default m-b-30" id="telegram-invite-history-card">
					<div class="card-header d-flex justify-content-between align-items-center">
						...
						<div class="d-flex align-items-center gap-2">
							<div id="telegram-invite-history-focus-meta" class="hint-text fs-12">Все invite-ссылки</div>
					...
					<table class="table table-hover m-b-0">
						<thead>
							<tr>
								...
								<th style="width:120px">К выплате</th>
						...
						<tbody id="merchant-orders-tbody">
							<tr>
								<td colspan="12" class="text-center text-muted p-t-25 p-b-25">Нет данных.</td>
							</tr>
						</tbody>
					</table>
				</div>
...
add_action(
	'wp_footer',
	function () use ( $nonce_list, $nonce_save, $nonce_status, $nonce_invite, $nonce_ledger, $nonce_orders, $nonce_delete, $is_root, $can_create, $can_edit, $can_block, $can_invite, $can_ledger, $can_orders, $companies_payload, $referrers_by_company, $current_company_id, $telegram_invite_status ) {
?>
<style>
...
(function ($) {
	...
	function renderActionMenu(row) {
		var hasSecondaryActions = CAN_INVITE || CAN_LEDGER || CAN_BLOCK || IS_ROOT;
		var html = '';
		...
		if (IS_ROOT) {
			html += '<div class="dropdown-divider"></div>';
			html += '<a href="#" class="dropdown-item text-danger js-merchant-delete" data-id="' + row.id + '" data-name="' + escHtml(row.name || ('Merchant #' + row.id)) + '">Удалить физически</a>';
		}
		html += '</div></div>';
		return html;
	}
	...
	function renderOrdersTable(rows) {
		...
		if (!rows.length) {
			$tbody.html('<tr><td colspan="12" class="text-center text-muted p-t-25 p-b-25">Ордеров пока нет.</td></tr>');
			return;
		}
		rows.forEach(function (row) {
			$tbody.append(
				...
				+ '<td>' + escHtml(row.merchant_payable_label || '—') + '</td>'
	...
	$(document).on('click', '.js-merchant-delete', function (e) {
		e.preventDefault();

		var merchantId = parseInt($(this).data('id'), 10) || 0;
		var merchantName = $(this).data('name') || ('Merchant #' + merchantId);
		if (!merchantId) {
			return;
		}
