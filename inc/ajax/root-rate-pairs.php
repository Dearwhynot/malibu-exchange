...
function crm_root_get_company_pairs_matrix(): array {
	...
	foreach ( $existing as $row ) {
		$normalized_code = function_exists( 'crm_normalize_legacy_pair_code' )
			? crm_normalize_legacy_pair_code( (string) $row->code )
			: (string) $row->code;
		$by_org_code[ (int) $row->organization_id ][ $normalized_code ] = $row;
	}
...
function me_ajax_root_rate_pair_save(): void {
	...
	$code       = function_exists( 'crm_normalize_legacy_pair_code' )
		? crm_normalize_legacy_pair_code( $code )
		: $code;
	...
	$result = function_exists( 'crm_company_set_exchange_pair_enabled' )
		? crm_company_set_exchange_pair_enabled( $company_id, $code, $is_active )
		: new WP_Error( 'contours_helper_missing', 'Не подключён helper company contours.' );

	if ( is_wp_error( $result ) ) {
		error_log( '[root-rate-pairs.ajax] 500 contour helper failed | code=' . $code . ' | message=' . $result->get_error_message() );
		wp_send_json_error( [ 'message' => $result->get_error_message() ], 500 );
	}

	$pair_id = (int) ( $result['pair_id'] ?? 0 );
	if ( $pair_id <= 0 && $is_active ) {
		wp_send_json_error( [ 'message' => 'Не удалось определить pair_id после сохранения пары.' ], 500 );
	}
