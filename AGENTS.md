...
## Создание новых страниц WordPress

При добавлении **новой страницы WordPress** не создавать её вручную через админку и не рассчитывать на ручное наполнение после деплоя.

### Обязательное правило
Если функционал требует появления новой страницы, агент должен:
- создать соответствующий шаблон / логику страницы в коде;
- зарегистрировать создание страницы **через существующую систему миграций проекта**;
- обеспечить, чтобы страница автоматически создавалась в базе данных при выполнении миграции;
- использовать стабильные и предсказуемые параметры страницы: `post_title`, `post_name` (slug), `post_status`, `post_type`, `post_template` и другие необходимые поля;
- предусмотреть защиту от дублей, чтобы одна и та же страница не создавалась повторно при повторном запуске миграции;
- по возможности привязывать страницу к конкретному шаблону сразу в миграции.

### Важно
- Не создавать страницы вручную как часть “инструкции для пользователя”.
- Не оставлять создание страницы на потом.
- Любая новая системная, служебная или пользовательская страница, требуемая модулем, должна поставляться вместе с кодом и создаваться автоматически через миграцию.

### Ожидаемый результат
После деплоя и выполнения миграций нужная страница уже существует в WordPress и готова к использованию без ручных действий.
